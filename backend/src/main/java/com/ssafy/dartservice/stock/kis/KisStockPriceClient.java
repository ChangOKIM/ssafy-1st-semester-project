...
import com.ssafy.dartservice.stock.StockPriceProvider;
...
public class KisStockPriceClient implements StockPriceProvider {
    ...
    public StockPriceResponseDto fetch(String stockCode) {
        ...
        if (body == null || body.getOutput() == null || !"0".equals(body.getRtCd())) {
            throw new IllegalStateException("KIS 시세 조회 실패: " + (body == null ? "empty response" : body.getMsg1()));
        }

        KisPriceResponse.Output output = body.getOutput();
        return new StockPriceResponseDto(
                output.getStckPrpr(), output.getPrdyVrss(), output.getPrdyCtrt(), output.getAcmlVol(),
                output.getW52Hgpr(), output.getW52Lwpr(), output.getPer(), output.getPbr(),
                output.getEps(), output.getBps());
    }
}
