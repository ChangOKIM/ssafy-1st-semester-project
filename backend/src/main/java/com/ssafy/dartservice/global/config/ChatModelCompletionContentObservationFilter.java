...
public class ChatModelCompletionContentObservationFilter implements ObservationFilter {
    ...
    public Observation.Context map(Observation.Context context) {
        ...
        chatModelObservationContext.addHighCardinalityKeyValue(new KeyValue() {
            @Override public String getKey() { return "gen_ai.prompt"; }
            @Override public String getValue() { return ObservabilityHelper.concatenateStrings(prompts); }
        });

        chatModelObservationContext.addHighCardinalityKeyValue(new KeyValue() {
            @Override public String getKey() { return "gen_ai.completion"; }
            @Override public String getValue() { return ObservabilityHelper.concatenateStrings(completions); }
        });
    ...
    private List<String> processPrompts(ChatModelObservationContext ctx) {
        return CollectionUtils.isEmpty(ctx.getRequest().getInstructions())
                ? List.of()
    ...
    private List<String> processCompletion(ChatModelObservationContext ctx) {
        if (ctx.getResponse() == null
                || ctx.getResponse().getResults() == null
